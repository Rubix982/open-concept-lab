...
class QualityMetrics:
    ...
    def relevance_score(self) -> float:
        """Alias for utilization or 1-hallucination."""
...
class ContextQualityMonitor:
    ...
    async def evaluate_retrieval(
        ...
        relevant_retrieved = [
            c for c in retrieved_contexts if c.get("id") in ground_truth["relevant_ids"]
        ]
        ...
        recall = (
            len(relevant_retrieved) / len(ground_truth["relevant_ids"])
            if ground_truth["relevant_ids"]
            else 0
        )
    ...
    async def monitor_context_usage(
        ...
        return QualityMetrics(utilization=utilization_rate, hallucination_rate=hallucination_rate)
    ...
    def evaluate_context(
        ...
        """
        Evaluate context quality based on query, contexts, and response.

        This is a simplified wrapper for monitor_context_usage that accepts
        string contexts instead of dicts.
        """
        # Convert string contexts to dicts
        context_dicts = [{"text": c, "id": str(i)} for i, c in enumerate(contexts)]

        # We don't have attention weights here, so we use overlap estimation
        # Run async method synchronously if needed, but since this is called from async,
        # we should probably make this async. However, EnhancedLLM calls it synchronously?
        # EnhancedLLM calls it as: quality_metrics = self.quality_monitor.evaluate_context(...)
        # It does NOT await it. So this must be synchronous.

        # Estimate utilization via overlap
        utilization_rate = self._estimate_utilization_via_overlap(context_dicts, response)
        self.metrics["context_utilization"].append(utilization_rate)

        # Hallucination detection is async, so we skip it or run a sync version
        # For now, let's just use utilization as a proxy for relevance

        return QualityMetrics(utilization=utilization_rate)

    def _estimate_utilization_via_overlap(
        self, contexts: list[dict[str, Any]], response: str
    ) -> float:
    ...
    async def _detect_hallucinations(self, contexts: list[dict[str, Any]], response: str) -> float:
        """
        ...
        context_text = " ".join([c.get("content", c.get("text", "")) for c in contexts]).lower()
