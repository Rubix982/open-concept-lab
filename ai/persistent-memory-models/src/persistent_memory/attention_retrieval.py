...
class AttentionEnhancedRetrieval:
    ...
    def retrieve_with_attention(
        ...
        for idx in top_chunk_indices:
            ...
            chunk_sentence_indices = [i for i, c in enumerate(sentence_to_chunk) if c == idx]
            if chunk_sentence_indices:
                chunk_sentence_attn = sentence_attention[chunk_sentence_indices]
                top_sentence_idx = chunk_sentence_indices[np.argmax(chunk_sentence_attn)]
...
class AttentionDataset:
    ...
    def add_example(self, query: str, contexts: list[str], relevance_scores: list[float]):
        """
        ...
        """
        self.examples.append({"query": query, "contexts": contexts, "relevance": relevance_scores})
