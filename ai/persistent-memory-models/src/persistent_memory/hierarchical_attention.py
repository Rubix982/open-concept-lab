...
class ChunkLevelAttention(nn.Module):
    ...
    def __init__(self, config: AttentionConfig):
        ...
        self.attention = MultiHeadAttention(config.embedding_dim, config.num_heads, config.dropout)
    ...
    def forward(
        ...
        chunk_attention = torch_functional.softmax(chunk_scores / self.config.temperature, dim=-1)

        # Weighted sum of chunks
        weighted_chunks = torch.bmm(chunk_attention.unsqueeze(1), chunk_embeddings).squeeze(1)
