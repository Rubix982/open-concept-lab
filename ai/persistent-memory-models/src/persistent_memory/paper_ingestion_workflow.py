...
async def fetch_papers_activity(params: PaperIngestionParams) -> List[Dict[str, Any]]:
    ...
    from persistent_memory.arxiv_downloader import (
        ArxivDownloader,
        get_curated_papers
    )
    ...
    try:
        ...
        if not paper_data or not paper_data.get('text'):
            return {
                'success': False,
                'arxiv_id': arxiv_id,
                'error': 'Failed to get paper data'
            }
        ...
        logger.info(f"Processed paper {arxiv_id}: {len(chunks)} chunks (cached: {repo.is_cached(arxiv_id)})")
...
async def extract_facts_from_paper_activity(paper_result: Dict[str, Any]) -> List[Dict]:
    ...
    try:
        ...
        paper_chunks = [
            r for r in results 
            if r.get('metadata', {}).get('arxiv_id') == arxiv_id
        ]
