    ...
    def allocate_context(
        ...
        context = {
            ...
            'retrieved': self._select_top_contexts(
                available_contexts,
                max_tokens=strategy.retrieved_context
            ),
            'history': self._get_recent_history(
                conversation_history or [],
                max_tokens=strategy.conversation_history
            ),
            'metadata': {
...
Use the provided context to answer questions accurately and comprehensively."""
    ...
    def _get_recent_history(
        ...
        logger.debug(f"Selected {len(selected)}/{len(history)} history turns ({current_tokens} tokens)")
        return selected
