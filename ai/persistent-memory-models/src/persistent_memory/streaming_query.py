...
class StreamingQueryEngine:
    ...
    async def stream_query(self, query: str, k: int = 5) -> AsyncGenerator[dict[str, Any], None]:
        """
        ...
        try:
            ...
            yield {"type": "status", "data": {"message": "Generating answer...", "progress": 0.8}}

            # Allocate context intelligently
            allocated_context, buffer_size = self.allocator.allocate_context(query, vector_results)
...
async def demo_streaming():
    ...
    async for chunk in engine.stream_query("Who is Elizabeth Bennet?"):
        ...
        if chunk_type == "status":
            ...
            print(f"🔗 Graph: {data['subject']} → {data['predicate']} → {data['object']}")
