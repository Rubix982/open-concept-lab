...
def ingest_conference(
    ...
    try:
        ...
        papers = connector.get_conference_papers(
            name.lower(), year, max_results=max_papers
        )
        ...
        print(
            f"   Papers processed: {result['papers_processed']}/{result['total_papers']}"
        )
...
def query(query_text: str, k: int = 5):
    ...
    for result in vector_results:
        ...
        if metadata.get("arxiv_id"):
            print(
                f"   Source: {metadata.get('title', 'Unknown')} ({metadata['arxiv_id']})"
            )
        print()
