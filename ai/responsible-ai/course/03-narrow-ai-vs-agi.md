...
graph TD
    subgraph PARADIGMS["AI Paradigms"]
        ...
        AGI["AGI\n(general-purpose)"]
        SYM["Symbolic AI\n(rules + logic)"]
        NEU["Neural / Connectionist\n(learned from data)"]
        GEN["Generative Models\n(Claude, ChatGPT, Midjourney)"]
    end

    subgraph CRITIQUE_CONCEPT["Critique I — Conceptual"]
        CAT["Category Error\n(is general intelligence coherent?)"]
        DEF["No Agreed Definition\n(narrative, not target)"]
    end

    subgraph CRITIQUE_CAPITAL["Critique II — Economic"]
        CAP["Capital Concentration"]
        UND["Underconsumption Problem"]
        UBI["UBI — gestured at, not designed"]
        REG["Regulatory Capture\n(urgency as weapon)"]
    end

    subgraph KSA["Case Study — Disconnected from Struggle"]
        OUT["Productive economy outsourced"]
        SUS["Culture suspended, not preserved"]
        BIN["Binary: tradition vs. imported modernity"]
        DIS["Dislocation when external pressure arrives"]
        VIC["Youth are victims of the policy, not the problem"]
    end

    subgraph CRITIQUE_ANTHRO["Critique III — Anthropological"]
        HED["Hedonic Treadmill"]
        FLO["Flow States\n(Csikszentmihalyi)"]
        ANT["Antifragility\n(Taleb)"]
        MEA["Meaning requires difficulty\n(Frankl)"]
    end

    subgraph DUNE["The Dune Lens"]
        BJ["Butlerian Jihad\n(war on thinking machines)"]
        MEN["Mentats\n(cultivate human cognition)"]
        SPI["Spice = Compute\n(dependency on controlled resource)"]
        FRE["Fremen\n(constraint as strength)"]
    end

    subgraph REFRAME["The Real Question"]
        RQ["Which difficulty is worth preserving?\nWhich is unnecessary suffering?"]
    end
