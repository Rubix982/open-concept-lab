"""E-025 · Type overlap or semantic relatedness? [E-024]'s two points do not separate them.

[E-024] found a birthplace edit damages citizenship 4x harder than occupation. Citizenship
is both type-matched to the edit (a country) and semantically related to it (about origin),
so two different accounts fit. Language separates them: semantically related to country of
origin, typed as a language.

    type overlap      -> language patterns with OCCUPATION (~1.6 nats)
    semantic related  -> language patterns with CITIZENSHIP (~6.6 nats)

Identical measure and control to [E-024]; one probe added. Run as its own ticket rather
than by editing [E-024] so the published numbers keep the code that produced them.
...
PROBES = {
    ...
    # The discriminator: related to origin, but its answers are languages, not countries.
    "language":    ("P1412", "The native language of {} is"),
}
...
def main() -> None:
    ...
    log = setup("run_e025", config={
        "ticket": "E-025", "model": MODEL, "layer": LAYER, "n_subjects": len(items),
        "probes": {k: v[1] for k, v in PROBES.items()},
        "measure": "teacher-forced log P(true answer), paired, no candidate pool"})
    log.info("%d subjects with all %d attributes labelled", len(items), len(PROBES))

    m = connect(MODEL)
    cache = ROOT / "results" / "cache" / f"E025_s{args.seed}.json"
    ...
    d = {k: summary[f"edited|{k}"]["mean_drop"] - summary[f"control|{k}"]["mean_drop"]
         for k in PROBES}
    ...
    if lang_base > -0.35:
        log.warning("language baseline is %.2f — near ceiling, so a small drop may be a "
                    "floor effect rather than a null. Treat as uninterpretable.", lang_base)
    span = d["citizenship"] - d["occupation"]
    if span < 0.5:
        log.info("the E-024 gradient did not reproduce; nothing to discriminate")
    else:
        pos = (d["language"] - d["occupation"]) / span
        log.info("language sits %.0f%% of the way from occupation to citizenship", 100 * pos)
        if pos < 0.33:
            log.info("TYPE OVERLAP: language patterns with occupation. The gradient is "
                     "competition for the probe's answer TYPE, not semantic relatedness.")
        elif pos > 0.67:
            log.info("SEMANTIC RELATEDNESS: language patterns with citizenship, despite "
                     "sharing no type with the injected value.")
        else:
            log.info("BOTH CONTRIBUTE: language lands between. Type overlap explains "
                     "%.0f%% of the gap and relatedness the rest.", 100 * (1 - pos))

    out = ROOT / "results" / "E-025-language.json"
    out.write_text(json.dumps({"ticket": "E-025", "probes": {k: v[1] for k, v in PROBES.items()},
